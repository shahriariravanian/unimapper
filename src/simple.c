...
static int
compress(float* array, size_t nx, size_t ny, size_t nz, float tolerance, zfp_bool decompress)
{
...
/*  zfp_stream_set_precision(zfp, precision); */
  ...
  if (decompress) {
    ...
    if (!zfp_decompress(zfp, field)) {
      ...
      printf("%ld\n", zfpsize);
      // fwrite(array, sizeof(float), zfp_field_size(field, NULL), stdout);
    }
  }
  else {
...
int main(int argc, char* argv[])
{
  ...
  /* compress or decompress array */
  status = compress(array, nx, ny, nz, 1e-3, decompress);
  printf("%d\n", status);
}
